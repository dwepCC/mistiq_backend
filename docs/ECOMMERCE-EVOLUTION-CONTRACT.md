...
Verificación: 2 tests de servicio nuevos + 2 modificados sobre Deuda #8 (`TestOrderEntregadoADevuelto_SincronizaDispatch` reemplaza al test que documentaba el gap, `TestOrderEntregadoADevuelto_SinDispatch_NoFalla`, `TestOrderEntregadoADevuelto_DobleClick_RechazadoLimpio`), `TestFindOrderTransition_TablaAprobada` actualizado (3 casos nuevos `false` por el bug corregido), 2 tests de concurrencia real contra MySQL nuevos (`TestUpdateOrderStatus_Devuelto_ConcurrentPost_SoloUnaTransicionExitosa`, `TestUpdateOrderStatus_Confirmado_ConcurrentPost_SoloUnaTransicionExitosa`) — todos en verde, junto con los 3 tests de concurrencia preexistentes de Fase 8/9 re-verificados sin regresión. `go build`/`go vet` limpios en todo el repo. `go test ./...`: mismas 4 fallas preexistentes de Greenter XML (billing/prepayment), confirmadas no relacionadas, sin fallas nuevas.
