...
Estado: **Fase 1, Fase 1.5, Fase 2, Fase 3, Fase 4, Fase 5, Fase 6, Fase 7, Fase 8, Fase 9, Fase 10 y Fase 11: APROBADAS.**

## ECOMMERCE V1 — COMPLETADO Y CERRADO

Fase 11 (auditoría integral, hardening y cierre) confirmó que el flujo CATÁLOGO→PRODUCTO→CARRITO→CHECKOUT→PEDIDO→WHATSAPP→CONFIRMADO→EN_PREPARACION→EMPAQUETADO→LISTO_PARA_DESPACHO→DESPACHADO→EN_TRANSITO→ENTREGADO (con DEVUELTO como salida) es coherente de extremo a extremo, corrigió las inconsistencias reales encontradas (ver bitácora de Fase 11) y cerró la Deuda técnica #8. GRE queda fuera de integración con Ecommerce V1 — las Guías de Remisión se continúan gestionando manualmente desde Billing → Guías; no existe integración Ecommerce → GRE en V1. Cualquier funcionalidad nueva a partir de acá (pagos online, reserva de stock, cupones, promociones, GPS/ETA, webhooks de carriers, catálogo de transportistas, tracking público, TMS/WMS, GRE automática, CRM, campañas, email/SMS/push, etc.) es una decisión de negocio nueva para una futura iniciativa (Ecommerce V2), no una fase automática de este contrato.

Aprobación general recibida sobre la v2, cerrando las decisiones pendientes: variantes vía `TenantProductPresentation` con nombre compuesto (sin nueva infraestructura), `PaymentStatus` como marcador inerte no editable por el cliente público, y `ecommerce.orders` como permiso legacy deprecado (sin retirar, sin nuevas asignaciones, sin inferencia automática sobre roles personalizados). Ver bitácora de fases al final del documento para el estado real de avance.
...
**Auditoría de separación de dominios (Parte D)**: confirmado que Pedido/Venta/Inventario/Dispatch/Cliente/Notificaciones/GRE siguen completamente separados. `ConvertToSale` sigue sin depender del estado logístico. Crear pedido, preparar, empaquetar, despachar, hacer tracking y entregar NO tocan `TenantProductStock`/`TenantStockMovement` (venta sigue siendo el único punto de movimiento de inventario). GRE sigue sin ningún puente con el despacho ecommerce.

**Bug real encontrado y corregido — bypass del dominio Dispatch vía el endpoint genérico (Parte C/M, "el código contradice una decisión ya aprobada")**: `orderTransitions` (`internal/ecommerce/service/order_status.go`) todavía contenía las filas `LISTO_PARA_DESPACHO→DESPACHADO`, `DESPACHADO→DESPACHADO` y `DESPACHADO→ENTREGADO`, heredadas de un diseño anterior a Fase 8. `UpdateOrderStatusAPI` (`PUT /orders/:id/status`, el endpoint GENÉRICO de transición) nunca filtró esos estados — así que cualquier usuario con `ecommerce.orders_dispatch` podía, por esa vía, mover un pedido a `DESPACHADO` sin crear ningún `TenantEcommerceDispatch` (bypass de `CreateDispatch`, Fase 8), o de `DESPACHADO` directo a `ENTREGADO` saltándose `EN_TRANSITO` y sin tocar el Dispatch en absoluto (bypass de `MarkDispatchDelivered`, Fase 9) — contradiciendo directamente la arquitectura aprobada en Fase 8/9 de que esas dos transiciones se alcanzan EXCLUSIVAMENTE a través de sus servicios dedicados. El frontend nunca ofrecía estos botones (verificado en `orderTransitions.ts`), así que no era explotable desde el panel normal, pero el backend es la autoridad real y sí lo permitía vía API directa. **Corrección**: se eliminaron esas 3 filas de `orderTransitions`; `CreateDispatch`/`MarkDispatchDelivered` siguen actualizando `Order.Status` directamente, sin pasar por esta tabla (sin cambios en ellos). Archivo: [order_status.go](../internal/ecommerce/service/order_status.go). Test: `TestFindOrderTransition_TablaAprobada` actualizado con los 3 casos ahora `false`.

**Deuda #8 cerrada — `Dispatch.Status` se sincroniza con `DEVUELTO` (Parte E, prioridad explícita)**: auditados `UpdateOrderStatus`, `OrderStatusHistory`, `Dispatch`, `DispatchStatusHistory`, permisos, transacciones, concurrencia y frontend. Solución: dentro de la MISMA transacción de `UpdateOrderStatus`, cuando el pedido pasa a `DEVUELTO`, se busca el `Dispatch` asociado (`SELECT...FOR UPDATE`) y, si existe y sigue en `ENTREGADO`, se sincroniza a `DEVUELTO` y se escribe su propio `TenantEcommerceDispatchStatusHistory` — nunca se mezcla con el historial del pedido, los dos dominios se mantienen separados. No es una segunda máquina de estados: es la misma decisión que ya rige `CreateDispatch`/`MarkDispatchDelivered` (un cambio en un dominio empuja, cuando corresponde, el cambio correspondiente en el otro dentro de la misma transacción). Casos cubiertos: pedido sin Dispatch (no falla, no hay nada que sincronizar), Dispatch que ya no está en `ENTREGADO` (defensa en profundidad, no se fuerza el salto), doble click/retry (idempotente por construcción — el pedido solo llega a `DEVUELTO` una vez, no existe `DEVUELTO→DEVUELTO` en la tabla), concurrencia real (`SELECT...FOR UPDATE` en Order y Dispatch, verificado con MySQL real: 20 requests simultáneos → 1 éxito + 19 rechazos limpios, exactamente 1 fila de historial en cada dominio). El frontend YA soportaba `DEVUELTO` en `Dispatch.Status` desde Fase 9 (label/badge/progreso en `PedidoWebDetailModal.tsx` y `AccountOrderDetailPage.tsx`) como código defensivo nunca antes alcanzable — no requirió ningún cambio. Archivos: [ecommerce_service.go](../internal/ecommerce/service/ecommerce_service.go) (`syncDispatchOnOrderReturned`). Tests: `TestOrderEntregadoADevuelto_SincronizaDispatch`, `TestOrderEntregadoADevuelto_SinDispatch_NoFalla`, `TestOrderEntregadoADevuelto_DobleClick_RechazadoLimpio`, `TestUpdateOrderStatus_Devuelto_ConcurrentPost_SoloUnaTransicionExitosa` (MySQL real).

**Hardening de concurrencia general en `UpdateOrderStatus` (Parte H, hallazgo real durante la auditoría de idempotencia)**: antes de Fase 11, el pedido se leía FUERA de la transacción y se actualizaba con un `UPDATE` ciego adentro, sin `SELECT...FOR UPDATE` — dos requests concurrentes sobre el MISMO pedido (p. ej. doble click en "Confirmar") podían leer el mismo estado viejo, pasar ambas la validación, y las dos escribir historial, duplicando `TenantEcommerceOrderStatusHistory`. Se corrigió con el mismo patrón `SELECT...FOR UPDATE` ya usado en `CreateDispatch`/`MarkDispatchInTransit`/`MarkDispatchDelivered` (Fase 1.5/8/9): el pedido se bloquea y se revalida DENTRO de la transacción para TODAS las transiciones, no solo para Devolución. Verificado con MySQL real (`TestUpdateOrderStatus_Confirmado_ConcurrentPost_SoloUnaTransicionExitosa`: 20 requests simultáneos `PENDIENTE→CONFIRMADO` → 1 éxito + 19 rechazos limpios, exactamente 1 fila de historial). No cambia ningún mensaje de error ni comportamiento observable para el caso no concurrente — mismo contrato, solo se cerró la ventana de carrera.

**Deuda #6 auditada — permisos legacy (Parte E)**: búsqueda de `ecommerce.orders` (el permiso único, no los granulares `orders_*`) en Sidebar, AppRouter, rutas, componentes, backend, servicios, seed, permisos y documentación. Resultado: ya no gatea NADA funcional — `Sidebar.tsx` usa `ecommerce.orders_view`, `routes.go` usa los 6 permisos granulares, `role_service.go` ya no lo siembra para tenants nuevos (`SeedPermissions`). Las únicas referencias que quedan son comentarios explicando la deprecación, el label legacy en `permissionLabels.ts` (para tenants que ya lo tenían de antes) y la migración histórica `v138`. **No se encontró ningún punto funcional que dependiera incorrectamente del permiso legacy — no se corrigió nada porque no había nada que corregir.** `ecommerce.orders` permanece deprecado, sin retirar, sin nuevas asignaciones (decisión ya aprobada, sin cambios).

**Deuda #7 auditada**: confirmado que `useNotificationEvents.ts` (la implementación Ecommerce) ya está correctamente ligado a `isAuthenticated` (cerrado en Fase 6) — no tiene el problema de `useBillingEvents.ts`. `useBillingEvents.ts` sigue siendo una deuda de Billing, fuera de alcance de esta fase, sin tocar.

**Deudas #1, #2, #3, #4 auditadas, mantenidas fuera de alcance (Parte E, explícitamente no implementadas)**: revocación server-side de sesión (#1) y password reset (#2) no son bugs introducidos por esta evolución ni bloquean el cierre de V1 — son deuda transversal de autenticación, no se construyó ningún sistema de sesiones nuevo. Índices UNIQUE de stock (#3) y `adjustmentInWithSerials`/`adjustmentOutWithSerials` (#4) pertenecen a Inventario, no a Ecommerce — auditado que ninguna funcionalidad de Ecommerce introdujo o depende de esos riesgos; no se tocó ninguna migración de índices ni el código muerto de series.

**RBAC (Parte F) — verificado contra la matriz exacta pedida**, backend y frontend, no solo botones ocultos:

| Rol | Ver | Preparar | Gestionar | Convertir | Despachar | Devolver |
|---|---|---|---|---|---|---|
| Almacenero | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Vendedor | ✅ | ❌ | ✅ | ✅ | ❌ | ❌ |
| Supervisor | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
Coincide exactamente con `defaultRolePermissions` (`internal/users/service/role_service.go`) — sin cambios, ya estaba correcto desde las fases anteriores. Backend: cada endpoint exige su permiso vía middleware de ruta (`routes.go`) o revalidación específica de transición (`UpdateOrderStatusAPI`). Frontend: `orderTransitions.ts`/`PedidoWebDetailModal.tsx` solo ocultan botones, nunca reemplazan la validación real.

**Multi-tenant (Parte G)**: sin cambios de código — la arquitectura sigue siendo una base de datos MySQL por tenant (aislamiento estructural, no por columna `tenant_id`), verificada extensamente con tests reales en Fases 4/8/9/10 (`TestCreateDispatch_TenantIsolation`, `TestMarkDispatchInTransit/Delivered_TenantIsolation`, `TestGetCustomerOrder_TenantIsolation`, `TestBuildLabelDataForOrder_TenantIsolation`, etc.). `syncDispatchOnOrderReturned` (Fase 11) opera dentro de la misma transacción/conexión ya aislada por tenant — mismo criterio, no requirió un test nuevo de aislamiento.

**Idempotencia y concurrencia (Parte H)**: auditadas `CreateOrder`, `ConvertToSale`, `Confirm/Prepare/Pack/ReadyForDispatch` (unificadas en `UpdateOrderStatus`), `CreateDispatch`, `UpdateDispatchMetadata`, `MarkInTransit`, `MarkDelivered`, `Return`, `GenerateLabel`. Todas menos `UpdateDispatchMetadata` (PATCH puro, sin condición de carrera posible al no depender de un estado previo) usan `SELECT...FOR UPDATE` dentro de una transacción — incluyendo, desde esta fase, `UpdateOrderStatus`/`Return` (antes no lo tenían, ver hallazgo arriba). `GenerateLabel` es de solo lectura, confirmado sin efectos secundarios incluso con reimpresiones repetidas.

**Compatibilidad histórica (Parte I)**: sin cambios — `ItemsJSON` sigue como fallback junto a `OrderItems` (`ecommerce_service.go`, `convert.go`, `print_data.go`, y sus tests), pedidos de invitado/cliente/convertidos/no convertidos/sin Dispatch siguen abriéndose y procesándose sin romperse. No se eliminó ni volvió a migrar nada.

**Frontend (Parte J)**: auditado buscando `TODO`/`FIXME`/`legacy`/`deprecated`/`ecommerce.orders`/`shipping-label`/`carrier`/`ItemsJSON`/`status`/`dispatch` en `PedidosWebPage`, `PedidoWebDetailModal`, cuenta del cliente, tracking, notificaciones, checkout, carrito, despacho, etiqueta. No se encontró ningún TODO/FIXME real ni inconsistencia — los matches eran comentarios de diseño ya correctos (incluyendo el manejo defensivo de `DEVUELTO` en Dispatch que ya existía sin usarse, confirmado arriba). No se agregó UX nueva.

**No implementado, exactamente como se pidió (Parte M)**: ningún pago online, pasarela, reserva de stock, HOLD, cupón, promoción, GPS/ETA, webhook de carrier, integración con transportista externo, catálogo de carriers, tracking público, TMS/WMS, picking persistente, GRE automática, CRM, campaña, email/SMS/push. Todo lo encontrado en la auditoría que no pertenecía al alcance de V1 (ninguno, en este caso — no surgió nada nuevo fuera de las deudas ya documentadas) queda registrado como futura iniciativa, no implementado.

Verificación: 3 tests de servicio nuevos sobre Deuda #8 (`TestOrderEntregadoADevuelto_SincronizaDispatch` reemplaza al test que documentaba el gap, `TestOrderEntregadoADevuelto_SinDispatch_NoFalla`, `TestOrderEntregadoADevuelto_DobleClick_RechazadoLimpio`), `TestFindOrderTransition_TablaAprobada` actualizado (3 casos nuevos `false` por el bug corregido), 2 tests de concurrencia real contra MySQL nuevos (`TestUpdateOrderStatus_Devuelto_ConcurrentPost_SoloUnaTransicionExitosa`, `TestUpdateOrderStatus_Confirmado_ConcurrentPost_SoloUnaTransicionExitosa`), y el E2E real de Fase 9 (`TestE2E_FlujoCompletoDespachoYTracking`) extendido con 7 pasos más (19-25) que continúan el MISMO pedido HTTP hasta la devolución: Dispatch sincronizado, ambos historiales sin mezclarse, doble click rechazado sin duplicar, el cliente viendo la devolución reflejada, stock/kardex intactos — todos en verde, junto con los 3 tests de concurrencia preexistentes de Fase 8/9 re-verificados sin regresión. `go build`/`go vet` limpios en todo el repo. `go test ./...`: mismas 4 fallas preexistentes de Greenter XML (billing/prepayment), confirmadas no relacionadas, sin fallas nuevas.

**Fuera de alcance de Ecommerce V1 — futura iniciativa (si el negocio la solicita)**: GRE automática/asistida para ecommerce (ver Fase 10), pagos online y reserva de stock asociada, cupones/promociones, tracking público/GPS/ETA/webhooks de transportistas externos, TMS/WMS, revocación server-side de sesión de cliente, password reset general del sistema, índices UNIQUE de stock. Ninguna de estas se implementó ni se diseñó en Fase 11 — quedan como decisión de negocio pendiente para una iniciativa futura (Ecommerce V2), no como una Fase 12 automática.
...
6. ~~**Gate de permisos legacy en Sidebar/rutas del panel**~~ — **RE-AUDITADA en Fase 11, confirmada sin problema**: búsqueda completa de `ecommerce.orders` (el permiso legacy) en Sidebar/rutas/componentes/backend/servicios/seed/documentación — no gatea ningún punto funcional del Ecommerce (Sidebar usa `orders_view`, rutas usan los 6 permisos granulares). Fuera de Ecommerce, otros módulos no se auditaron (fuera de alcance de este contrato).
7. **`useBillingEvents.ts` no cierra el SSE en logout de shell nativo** (encontrado en Fase 6, fuera de su alcance; re-confirmado en Fase 11 que `useNotificationEvents.ts` no tiene el mismo problema): deuda de Billing, no de Ecommerce — sin tocar.
8. ~~**`Dispatch.Status` nunca se sincroniza con `DEVUELTO`**~~ — **CERRADA en Fase 11**: `UpdateOrderStatus` ahora sincroniza `Dispatch.Status` a `DEVUELTO` (si el Dispatch existe y sigue `ENTREGADO`) dentro de la misma transacción que la devolución del pedido, con su propio `DispatchStatusHistory`. Ver bitácora de Fase 11 y `syncDispatchOnOrderReturned` en `ecommerce_service.go`.
9. ~~**GRE no integrada con despacho ecommerce**~~ — **Fuera de alcance de Ecommerce V1** (decisión de negocio confirmada en Fase 10, re-confirmada en Fase 11): las Guías de Remisión se continúan gestionando manualmente desde Billing → Guías. No existe integración Ecommerce → GRE en V1. Integración GRE futura, si el negocio posteriormente la solicita, es una decisión nueva para una iniciativa aparte (Ecommerce V2), no una continuación automática de este contrato.
