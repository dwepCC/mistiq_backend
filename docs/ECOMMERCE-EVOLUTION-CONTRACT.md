...
**Hallazgo y corrección de un bug real en v141 (Fase 6)**, encontrado durante la verificación de MySQL de esta fase: la migración v141 (`idx_tenant_notifications_type`) nunca se había probado contra la forma REAL que v139 le da a la tabla `tenant_notifications` en un tenant existente (el struct local de v139 no lleva tags gorm, así que MySQL tipa la columna `type` como `TEXT`/`LONGTEXT`) — crear un índice sobre esa columna sin longitud de prefijo falla con `Error 1170 (BLOB/TEXT column used in key specification without a key length)`. Reproducido exactamente (se recreó la tabla vía la migración v139 real, no una aproximación) y corregido cambiando el índice a `type(60)` (prefijo, válido tanto sobre `TEXT` como sobre `VARCHAR(60)`). v141 no se había desplegado a ningún entorno real todavía (todo el trabajo sigue local, sin push), así que se corrigió el archivo de la migración directamente en vez de agregar una migración de parche aparte.
...
**Inventario/GRE/etiquetas**: crear un despacho no toca stock/kardex (verificado con test real). No se conectó con `TenantGreCarrier` ni con ninguna guía de remisión SUNAT — eso queda para una fase específica de documentos, no esta. No se implementó ningún PDF logístico ni etiqueta.

Verificación: 15 tests nuevos de servicio (creación exitosa, rechazo desde los 9 estados incompatibles, pedido inexistente, tenant isolation, doble despacho, recojo sin carrier, envío conserva dirección, tracking/peso decimal/bultos, valores negativos rechazados, no toca stock/kardex, conversión a venta independiente, actualización de metadata) + 9 de handler (RBAC de los 4 roles, contrato de respuesta, transición inválida, PATCH solo metadata, doble click sin 500) + 1 test de concurrencia real contra MySQL, todos en verde. `go build`/`go vet` limpios en todo el repo (excepto las 4 fallas preexistentes de Greenter XML en billing/prepayment, confirmadas no relacionadas). Frontend: `tsc --noEmit` sin errores.
