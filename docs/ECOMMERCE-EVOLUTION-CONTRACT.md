...
Puntos identificados durante la auditoría de fases anteriores, confirmados y mantenidos en revisión de Fase 4 — registrar acá para que ninguno se pierda entre fases:
...
5. **Tipos legacy de `EcommerceOrder.status` en frontend** (`mistiq_tenant/src/services/ecommerce.service.ts`): sigue tipado con el enum viejo (`'nuevo' | 'atendido' | 'cerrado' | 'cancelado'`) en vez del real (`PENDIENTE...DEVUELTO`). **Esta deuda SÍ entra en el alcance de la Fase 5** (panel de pedidos), a diferencia de las 4 anteriores — se corrige ahí, no antes ni aislada.
