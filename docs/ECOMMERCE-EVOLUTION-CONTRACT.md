...
Estado: **Fase 1, Fase 1.5, Fase 2, Fase 3, Fase 4, Fase 5, Fase 6, Fase 7, Fase 8 y Fase 9: APROBADAS. Fase 10: PARCIAL — etiqueta logística implementada, GRE bloqueada por decisión de negocio pendiente (ver bitácora de Fase 10).** Aprobación general recibida sobre la v2, cerrando las decisiones pendientes: variantes vía `TenantProductPresentation` con nombre compuesto (sin nueva infraestructura), `PaymentStatus` como marcador inerte no editable por el cliente público, y `ecommerce.orders` como permiso legacy deprecado (sin retirar, sin nuevas asignaciones, sin inferencia automática sobre roles personalizados). Ver bitácora de fases al final del documento para el estado real de avance.
...
**Pendiente para Fase 10**: shipping labels (✅ implementado — ver bitácora de Fase 10), GRE/documentos SUNAT del despacho (bloqueado, decisión de negocio pendiente — ver bitácora de Fase 10).
...
**GRE — NO IMPLEMENTADA. Decisión de negocio pendiente, explícitamente no inventada.**

- *Situación actual*: el despacho ecommerce (`TenantEcommerceDispatch`) es completamente independiente de Venta (`TenantSale`) y de GRE (`TenantDespatch`) — ningún código los conecta hoy.
...
- *Opciones evaluadas*:
  1. **Automática**: crear la GRE sola al despachar, adivinando motivo de traslado/modalidad y usando `CarrierName`/`TrackingCode` como transportista. **Riesgo real de compliance** — SUNAT puede rechazar una guía con datos inventados/incompletos, y esto es un documento tributario legal, no un PDF interno.
  2. **Manual/asistida**: un botón que abre el flujo de GRE YA existente en Billing, prellenado con lo que sí se conoce (destinatario, ítems), pero el usuario debe completar/confirmar motivo/modalidad/documento de identidad del destinatario.
  3. **Sin integración por ahora**: quien necesite una GRE para un envío ecommerce la crea manualmente desde Billing → Guías, sin ningún puente con el panel de pedidos.
- *Impacto*: la opción automática es la única que no requiere decisión de negocio adicional, pero es la más riesgosa (documento tributario mal formado). Las opciones 2 y 3 son seguras pero requieren que el negocio decida primero: ¿el checkout ecommerce debe empezar a pedir RUC/DNI+ubigeo del destinatario para habilitar GRE?, ¿qué motivo de traslado aplica por defecto a un envío ecommerce?, ¿toda GRE de ecommerce necesita un `TenantContact` real (como ya exige el flujo de Billing) o se acepta un contacto sintético?
- *Recomendación técnica*: **Opción 2 (manual/asistida)** cuando el negocio decida que la necesita — reutiliza el pipeline SUNAT ya probado sin automatizar un documento fiscal, pero antes se necesitan las 3 respuestas de arriba. **No se implementó nada de esto en Fase 10** — ni automático ni manual — por no tener esas respuestas.
...
9. **GRE no integrada con despacho ecommerce** (Fase 10, bloqueada a propósito — ver bitácora de Fase 10): `TenantEcommerceDispatch` no tiene ningún puente con `TenantDespatch`/`CreateAndSendDespatch` (GRE SUNAT). Requiere decisión de negocio previa: ¿el checkout ecommerce debe pedir RUC/DNI+ubigeo del destinatario para habilitar GRE?, ¿qué motivo de traslado/modalidad aplica por defecto?, ¿toda GRE de ecommerce necesita un `TenantContact` real? Recomendación técnica ya registrada (integración manual/asistida reutilizando el flujo de Billing, nunca automática) — falta la decisión de negocio para poder implementarla.
